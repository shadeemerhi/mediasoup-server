...
app.get("*", (req, res, next) => {
  const path = '/sfu/';
    if (req.path.indexOf(path) == 0 && req.path.length > path.length)
        return next();
    res.send(
        "You need to specify a room name in the path e.g. 'http://localhost:3000/room'"
    );
});
      ...
      try {
        ...
        //   producerId: remoteProducerId,
          ...
          rtpCapabilities
            ...
            console.log('REMOTE PRODUCER ID', remoteProducerId, adminProducer.id);
          ...
          consumer.on('producerclose', () => {
            ...
            console.log('TRANSPORT LENGTH BEFORE', transports.length);
            transports = transports.filter(transportData => transportData.transport.id !== consumerTransport.id)
            console.log('TRANSPORT LENGTH AFTER', transports.length);
            consumer.close()
